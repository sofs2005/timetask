# timetask
一款支持自定义定时任务的GEWE版DOW插件，支持自定义时间、轮询周期、自定义时间，包含动态添加任务、取消任务、查看任务列表等功能，一款定时任务系统的插件。老版本请安装2.9版本。
...
## **【插件安装方法】**
1. clone本仓库 或 下载源代码
2. 将本插件的文件夹timetask, 放进 DOW 的 plugins 文件夹中（DOW启动后，会自动加载本插件）
3. 安装插件依赖库：进入 timetask 文件夹，执行命令：pip3 install -r requirements.txt
4. 至此，插件安转完毕，启动DOW，使用 timetask 插件
